...
| <a name="input_dataform_release_compilation_vars"></a> [dataform\_release\_compilation\_vars](#input\_dataform\_release\_compilation\_vars) | User variables passed to Dataform compilation, exposed as<br/>`dataform.projectConfig.vars` in sqlx/JS. Use to inject env-specific<br/>values (e.g. `{ env = var.env }`) when source/target databases or<br/>schemas are derived from the deployment environment. | `map(string)` | `{}` | no |
...
| <a name="input_dataform_release_schema_suffix"></a> [dataform\_release\_schema\_suffix](#input\_dataform\_release\_schema\_suffix) | Optional suffix appended to all dataset names at compile time. | `string` | `null` | no |
| <a name="input_dataform_release_table_prefix"></a> [dataform\_release\_table\_prefix](#input\_dataform\_release\_table\_prefix) | Optional prefix prepended to all table names at compile time. | `string` | `null` | no |
...
| <a name="input_service_account_email"></a> [service\_account\_email](#input\_service\_account\_email) | Custom service account email for Dataform act-as mode. When set, the Dataform<br/>repository runs workflows as this SA and it is granted dataEditor on all<br/>datasets managed by this module.<br/><br/>IAM prerequisites (not managed by this module):<br/>- Grant the Dataform service agent<br/>    service-<project\_number>@gcp-sa-dataform.iam.gserviceaccount.com<br/>  the roles roles/iam.serviceAccountTokenCreator and roles/iam.serviceAccountUser<br/>  on this custom SA.<br/>- Ensure this custom SA has roles/bigquery.jobUser on the project. | `string` | `null` | no |
