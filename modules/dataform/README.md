...
| [google-beta_google_dataform_repository_release_config.configs](https://registry.terraform.io/providers/hashicorp/google-beta/latest/docs/resources/google_dataform_repository_release_config) | resource |
...
| <a name="input_dataform_release_config_name"></a> [dataform\_release\_config\_name](#input\_dataform\_release\_config\_name) | (Deprecated: use dataform\_release\_configs instead) Dataform release config name | `string` | `null` | no |
| <a name="input_dataform_release_configs"></a> [dataform\_release\_configs](#input\_dataform\_release\_configs) | Map of release configs to create. Key is the release config name. Supports multiple branches/configs with individual settings. | <pre>map(object({<br/>    git_commitish    = string<br/>    cron_schedule    = optional(string, "0 5 * * *")<br/>    timezone         = optional(string, "UTC")<br/>    default_database = optional(string, null)    # If null, uses project_id<br/>    schema_suffix    = optional(string, null)    # Suffix to append to schema names<br/>    table_prefix     = optional(string, null)    # Prefix to add to table names<br/>    vars             = optional(map(string), {}) # Custom variables for compilation<br/>  }))</pre> | `{}` | no |
| <a name="input_dataform_release_cron_schedule"></a> [dataform\_release\_cron\_schedule](#input\_dataform\_release\_cron\_schedule) | (Deprecated: use dataform\_release\_configs instead) Cron schedule for dataform release config | <pre>object(<br/>    {<br/>      cron_schedule = optional(string, "0 5 * * *")<br/>      timezone      = optional(string, "UTC")<br/>    }<br/>  )</pre> | `{}` | no |
...
| <a name="input_dataform_workflows"></a> [dataform\_workflows](#input\_dataform\_workflows) | Dataform workflows to be created | <pre>map(object({<br/>    cron_schedule        = string<br/>    tags                 = list(string)<br/>    include_dependencies = optional(bool, false)<br/>    timezone             = optional(string, "UTC")<br/>    release_config       = optional(string, null) # Name of release config to use, null uses default/legacy behavior<br/>  }))</pre> | `{}` | no |
