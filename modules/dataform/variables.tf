...
variable "service_account_email" {
  ...
  description = <<-EOT
    ...
    repository runs workflows as this SA and it is granted dataEditor on all
    datasets managed by this module.

    IAM prerequisites (not managed by this module):
    - Grant the Dataform service agent
        service-<project_number>@gcp-sa-dataform.iam.gserviceaccount.com
      the roles roles/iam.serviceAccountTokenCreator and roles/iam.serviceAccountUser
      on this custom SA.
